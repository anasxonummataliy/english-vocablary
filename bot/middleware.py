...
class IsJoinChannelMiddleware(BaseMiddleware):
    ...
    async def __call__(self, handler,  event, data):
        if event.event.chat.type != ChatType.PRIVATE:  # type: ignore
            return await handler(event, data)
        bot: Bot = data['bot']
        user_id = event.event.from_user.id  # type:ignore
        unsubscribed_channels = []
        for channel_id in CHANNELS:  # type: ignore
            chat_member = await bot.get_chat_member(channel_id, user_id)
            if chat_member.status not in (ChatMemberStatus.MEMBER, ChatMemberStatus.CREATOR, ChatMemberStatus.ADMINISTRATOR):
